...
class HuggingfaceModel(BaseModel):
    ...
    def predict(self, input_data, temperature, return_full=False):
        ...
        elif full_answer.startswith(input_data[:int(0.9*len(input_data))]):
            ### 90% of the answr match we just need to find the answr 
            ## find the last occurance of Answer
            SHIFT_FOR_ANSWER_TXT = 9
            input_data_offset = full_answer.rfind("\nAnswer:") + SHIFT_FOR_ANSWER_TXT
        else:
            # print("")
            # raise ValueError('Have not tested this in a while.')   ### TODO: running issues here sometimes 
            logging.warning('Have not tested this in a while.')
            SHIFT_FOR_ANSWER_TXT = 9
            input_data_offset = full_answer.rfind("\nAnswer:") + SHIFT_FOR_ANSWER_TXT
